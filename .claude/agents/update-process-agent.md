...
- **RIPER-5 Process / tool adapters** → `process/development-protocols/` first, then `.claude/CLAUDE.md` or `AGENTS.md` if adapter guidance must change
...
- Run `ls process/features/` and compare to the **Current features** list in `.claude/CLAUDE.md` and `AGENTS.md`
...
- Route changes to the correct file:
  ...
  - Container/Docker/service changes → `container.md`
  ...
  - Skill runtime/app changes → `skill-apps.md` or `skills.md`
...
- If a context file exceeds roughly 800 lines and has separable subtopics, flag it for context group promotion and suggest `audit-context`.
...
- If the task changed context structure, routing, grouping, file moves, file splits, context discovery, or agent discoverability assumptions, you MUST treat this as an `audit-context` follow-up case, not just a normal context edit.
- In those cases, do both:
  - make the immediate required context/process edits for this task
  - explicitly trigger or recommend the `audit-context` skill before claiming the context layer is fully reconciled
...
- Explicitly check whether the task should trigger:
  - `generate-context` for `process/context/all-context.md` refresh
  - `audit-context` for routing/grouping/discoverability drift
  - `audit-plans` for stale active-plan reconciliation
- Use these as specialist follow-up surfaces rather than improvising replacements:
  - `generate-context` when the repo context router itself needs refresh
  - `audit-context` when context routing, grouping, discoverability, or structural context edits changed
  - `audit-plans` when stale active-plan reconciliation or session-close plan review is needed
- If structural context changes happened, `audit-context` is not optional housekeeping; it is the specialist validation step for the context layer.
...
- If `AGENTS.md` changes, verify whether `.claude/CLAUDE.md` must change too.
...
| `container.md` | Docker container lifecycle, plugin deployment, local dev commands, service ports |
...
| `skills.md` | Skill system context, skill.json format, deployment |
...
> "Session complete. Want me to run a plan audit to review what's done, what's in progress, and what's next? (follows the `audit-plans` skill)"

If user confirms, follow the `audit-plans` skill at `.agents/skills/audit-plans/SKILL.md` exactly.
...
- If cleanup debt is broader than the selected plan, suggest `audit-plans` as a follow-up maintenance step rather than silently normalizing unrelated plans.
...
- Established pattern for API profile router
...
2. Code Standards - process/development-protocols/implementation-standards.md
   Summary: Add API router co-location pattern
   Context: User-profiles implementation established this pattern
   Text to add: "API routers should co-locate validators..."
...
✅ 2. Updated process/development-protocols/implementation-standards.md with API router pattern
✅ 3. Updated all-context.md with API surface changes
✅ Created memory: "API Router Co-location Pattern"

**Phase 5: Final Review**

Changes made:
- Memory entries: 1 (API Router Co-location Pattern)
