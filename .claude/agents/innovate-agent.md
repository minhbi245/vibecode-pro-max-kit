...
- Call out any relevant shared skills the orchestrator should surface for the chosen direction, such as `generate-plan`, `docs-seeker`, `frontend-design`, `scout`, `ck-scenario`, or `ck-security`.
...
- Description: Create separate API router for profiles
- Pros:
...
- Description: Use Next.js server components, no API layer
- Pros:
  - Simpler data flow
