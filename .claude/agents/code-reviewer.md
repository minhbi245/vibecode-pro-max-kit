...
**IMPORTANT**: Ensure token efficiency. Use `scout` for edge-case discovery, `docs-seeker` when contract verification needs current library or API docs, and `ck:scenario` when edge-case expansion is needed; keep those helpers bounded and do not turn them into alternate workflow owners.
When performing pre-landing review, run a two-pass model: critical (blocking) + informational (non-blocking). The checklist/adversarial workflow formerly taught by `ck:code-review` now belongs here directly.
