...
- Using the `generate-plan` skill as the authoritative artifact contract before creating or updating a plan
...
When creating or updating a plan file, use the `generate-plan` skill at
`.agents/skills/generate-plan/SKILL.md` as the authoritative reference for
...
The `generate-plan` skill defines what the plan artifact must contain.
Planning rigor formerly taught by `ck:plan` now belongs in this pairing: use `generate-plan` for the artifact contract and keep adversarial validation, dependency mapping, and verification-gate thinking inside the plan itself instead of a parallel plan-owner workflow.
...
2. Generate database migration for Profile model
3. Create API router at packages/api/src/router/profile.ts
