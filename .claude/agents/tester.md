...
- `sequential-thinking` for complex verification reasoning
- `scout` for diff-to-test mapping and repo discovery
- `ck:debug` when failures need root-cause analysis
- `ck:scenario` for edge-case or adversarial coverage gaps
- `web-testing`, `chrome-devtools`, or `agent-browser` only when browser or runtime verification is actually the required surface
