...
4. If docs or API lookup is part of the task, surface `docs-seeker`; if plan creation or update is required, use `generate-plan` as the authoritative plan contract
5. Treat `generate-context` or `audit-context` as conditional helpers only when the request or repo truth indicates context drift
...
5. Create or update the implementation plan using the `generate-plan` skill contract only after plan-location choice is explicit
...
- Planning should follow the `generate-plan` skill's artifact rules
