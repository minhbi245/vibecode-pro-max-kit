...
- do not let `team` or any helper skip approval or phase-lock rules
- use `team` only for true parallel coordination with clear integration boundaries
- keep final execution ownership in the execute-agent unless the orchestrator explicitly re-routes the workflow

Execution checkpoints formerly taught by `ck:cook` now belong here:
...
- use `team` only when parallel coordination is truly needed, not as a separate execution owner
