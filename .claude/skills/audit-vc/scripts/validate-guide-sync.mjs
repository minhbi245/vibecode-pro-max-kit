...
import { parseFrontmatter, listSkillDirs, exists, abs } from "../../audit-context/scripts/shared-skill-utils.mjs";
...
if (!exists(guidePath)) {
  ...
  for (const folder of diskSkillFolders) {
    ...
    // Strip ck: prefix for matching (README.md uses folder names, not ck:-prefixed names)
    const nameWithoutPrefix = name.startsWith("ck:") ? name.slice(3) : name;
  ...
  for (const skill of guideSkills) {
    // Also check ck:-prefixed variant (README.md may list "code-reviewer" which is an agent, not a skill folder)
    if (!diskSkillFolders.has(skill) && !diskSkills.has(skill) && !diskSkills.has(`ck:${skill}`)) {
